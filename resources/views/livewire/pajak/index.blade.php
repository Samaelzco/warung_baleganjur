...
<section class="w-full">
    @php
        ...
        $usedTaxIds = Pesanan::query()
            ->whereIn('pajak_id', $items->getCollection()->pluck('id'))
            ->pluck('pajak_id')
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->flip();
    ...
    <div class="space-y-6">
        ...
        <div class="block sm:hidden">
            <div class="grid gap-3">
                @forelse($items as $p)
                    @php($isUsedInOrders = $usedTaxIds->has((int) $p->id))
                            ...
                            <div class="mt-2">
                                <flux:modal.trigger name="confirm-delete-pajak" class="w-full">
                                    <flux:button
                                        ...
                                        icon="trash"
                                        variant="danger"
                                        class="{{ $isUsedInOrders ? 'btn-disabled-muted' : 'btn-ghost-danger' }} w-full rounded-2xl shadow-sm transition whitespace-nowrap justify-center"
                                        wire:click="confirmDelete({{ $p->id }})"
                                        :disabled="$isUsedInOrders"
                                        title="{{ $isUsedInOrders ? __('This tax is already used in orders.') : __('Delete') }}"
                                    >
                                        {{ __('Delete') }}
                                    </flux:button>
                                </flux:modal.trigger>
                    ...
                    <table class="min-w-full table-fixed text-sm">
                        <thead>
                            <tr>
                                ...
                                <th class="border-b border-r border-neutral-200/80 px-6 py-4 text-center text-xs font-semibold uppercase tracking-[0.2em] text-neutral-500 dark:border-neutral-800/70 dark:text-neutral-400">{{ __('Rate') }}</th>
