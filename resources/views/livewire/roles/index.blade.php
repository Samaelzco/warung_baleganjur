...
<section class="w-full">
    @php
        ...
        $assignedRoleIds = DB::table('model_has_roles')
            ->whereIn('role_id', $items->pluck('id'))
            ->distinct()
            ->pluck('role_id')
        ...
        <div class="block sm:hidden">
            <div class="grid gap-3">
                @forelse ($items as $role)
                    @php($isAssignedToUsers = $assignedRoleIds->has((int) $role->id))
                            ...
                            @can('roles.manage')
                                ...
                                <flux:modal.trigger name="confirm-delete-role" class="flex-1">
                                    <flux:button
                                        ...
                                        icon="trash"
                                        variant="danger"
                                        class="{{ $isAssignedToUsers ? 'btn-disabled-muted' : '' }} w-full"
                                        wire:click="confirmDelete({{ $role->id }})"
                                        :disabled="$isAssignedToUsers"
                                    >
                                        {{ __('Delete') }}
                                    </flux:button>
                                </flux:modal.trigger>
                                            ...
                                            @can('roles.manage')
                                                ...
                                                <flux:modal.trigger name="confirm-delete-role-desktop">
                                                    <flux:button
                                                        ...
                                                        icon="trash"
                                                        variant="danger"
                                                        class="{{ $isAssignedToUsers ? 'btn-disabled-muted' : '' }} w-full rounded-2xl shadow-sm transition justify-center"
                                                        wire:click="confirmDelete({{ $role->id }})"
                                                        :disabled="$isAssignedToUsers"
                                                    >
                                                        {{ __('Delete') }}
