...
new class extends Component {
    ...
    public function setStatus(int $id, string $toStatus): void
    {
        ...
        if (!in_array($toStatus, $allowedTargets, true)) {
            $this->dispatch('kitchen-toast', message: __('No status change needed.'));
            return;
        }

        $pesanan->update(['status' => $toStatus]);

        $this->dispatch('kitchen-toast', message: __('Order status updated.'));
    }
    ...
    <div class="space-y-6">
        ...
        <div class="block sm:hidden">
            ...
            @forelse ($items as $item)
                @php($meta = $statusMeta[$item->status] ?? null)
                <div class="rounded-2xl border border-neutral-200/80 bg-white p-4 shadow-sm dark:border-neutral-800/70 dark:bg-neutral-900">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-base font-semibold text-neutral-900 dark:text-white">{{ $item->kode_pesanan }}</div>
