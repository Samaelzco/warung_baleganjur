...
new class extends Component {
    ...
    {
        abort_unless($pesanan->status === 'booking', 404);

        $pesanan->loadMissing(['details.addons:id']);
        $this->pesanan = $pesanan;
        $this->form = [
            'meja_id' => (string) $pesanan->meja_id,
            'customer_name' => (string) ($pesanan->customer_name === __('Guest') ? '' : ($pesanan->customer_name ?? '')),
            'customer_note' => (string) ($pesanan->customer_note ?? ''),
            'jumlah_orang' => (int) ($pesanan->jumlah_orang ?? 1),
            'subtotal' => (float) ($pesanan->subtotal ?? 0),
            'tax_total' => (float) ($pesanan->tax_total ?? 0),
            'total_harga' => (float) ($pesanan->total_harga ?? 0),
        ];

        $this->orderItems = $pesanan->details->map(fn ($detail) => [
            'menu_id' => (string) $detail->menu_id,
            'qty' => (int) $detail->qty,
            'harga' => (float) $detail->harga,
            'subtotal' => (float) $detail->subtotal,
            'addon_ids' => $detail->addons->pluck('id')->map(fn ($id) => (int) $id)->values()->all(),
        ])->values()->all();
    ...
    public function save(): void
    {
        ...
        DB::transaction(function () use ($validated, $items, $totals, $orderStatus) {
            $this->pesanan->forceFill([
                'meja_id' => $validated['meja_id'],
                'status_token' => $this->pesanan->status_token ?: $orderStatus->generateToken(),
            ...
            $this->pesanan->details()->each(function ($detail) {
            ...
            foreach ($items as $item) {
                $detail = $this->pesanan->details()->create([
        ...
        session()->flash('waiting_list_toast', __('Waiting list updated successfully.'));
...
<section class="w-full space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-1">
            <flux:heading size="xl" level="1">{{ __('Edit Waiting List') }}</flux:heading>
            <flux:subheading>{{ __('Update waiting list details and pre-ordered items.') }}</flux:subheading>
        </div>
                    ...
                    <div class="rounded-2xl border border-neutral-200/70 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-900">
                        ...
                        <div class="mt-3 space-y-3">
                            @foreach ($orderItems as $index => $item)
                                <div wire:key="edit-waiting-list-page-item-{{ $index }}" class="rounded-xl border border-neutral-200/70 bg-neutral-50/60 p-3 dark:border-neutral-800 dark:bg-neutral-950/40">
